import { format, isAfter, isPast, isToday, isTomorrow } from "date-fns";
import "../../styles/UpcomingEvent.css";
import "../../styles/Scrollbar.css";
import { useEffect, useState } from "react";

export default function UpcomingEvent({ eventsDataArray }) {
  ...
  function EventItem({ title, task, logo, date, style }) {
    ...
    var isTomorrowed = date && isAfter(date,today);
        ...
        <div className="w-full flex justify-between">
          ...
          {dateInfor && (
            <div className="flex flex-col items-end">
              <div
                className={`date ${
                  isTodayDate ? "warning" : isTomorrowed ? "tomorrow" : "error"
                }`}
              >
                {dateInfor[0]}
              </div>
  ...
  return (
    <div className="flex flex-col items-start  w-full h-full">
      ...
      <div className="w-full px-[1rem] mt-[1rem] h-full max-h-[25rem] overflow-y-scroll eventHolder scrollball-hidden">
        {events ? (
          events.map((event, index) => (
            <div key={index}>
              <EventItem
                ...
                date={event?.endDate}
                style={
                  index === events.length - 1 ? { borderLeftWidth: 0 } : {}
                }
              />
            </div>
          ))
